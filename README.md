...
- **Card picker** — My cards is an ordered, filterable wallet with a compact header add button. Adding pushes a dedicated page with live search, dynamically available issuer/category shelves, and a full-page custom-card step with a contained Cash back / Points selector, all backed by optimized local card art. Search keeps every match in a six-row scrolling region; issuer and category pages keep held cards visible and marked so several cards can be added without losing context.
...
- **Priority tie-break** — when two cards earn the same, your hand-set card order decides. During a drag, neighboring rows slide into their preview positions before you drop, the card and its drop position stay inside the visible list, and edge scrolling accelerates farther beyond the border up to a controlled cap; the reorder handle also supports the arrow keys.
...
- **Everything is editable** — issuers change rules constantly. Tap a held card row to override its rates or remove it, create a custom card with its bank and earn rates, or adjust point valuations in Settings.
