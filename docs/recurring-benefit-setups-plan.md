...
- Non-iOS browsers use native fixed positioning for the bottom navigation. On iOS, the navigation and Undo toast use absolute document positioning synchronized to the visual viewport’s page position, height, and width, bypassing the WebKit fixed-layer paint defect seen in Safari and Chrome. Tests cover live resize, scroll, and horizontally offset pinch-zoom geometry.
- The dependency-free browser harness passes 67 of 67 checks covering persistence, timing, full quarterly inventory, history compatibility, disclosure semantics, confirmation and removal, content-aware Undo sizing, visit boundaries, suspension, focus, mobile viewport anchoring, modal behavior, and copy-critical UI states.
...
- Earlier engineering, product, accessibility, and copy reviews passed with no remaining P0–P2 findings. The latest iOS absolute viewport anchor, scrolled-page behavior, horizontal pinch-zoom positioning, 200% zoom icon sizing, and toast clearance passed independent engineering and design/visual reviews with no remaining P0–P2 findings. Final confirmation on the user’s phone is still pending.
